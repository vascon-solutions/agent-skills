...
- When the app ships a dark mode, add a `[data-app-scheme="dark"]` `--app-*` block from the dark rows of the token map and a mock-only Scheme control that toggles `data-app-scheme` on the mock; the board chrome stays light.
