...
- Issuing byte-identical content again preserves the HTML and repairs missing index metadata using the original issue date. Existing digests and publication history are preserved; conflicting digests are rejected.
...
`issue` refuses when any check fails, when the embedded board id differs from the working filename, when a token row is `unresolved`, or when a new N is at or below any reserved or issued number. Identical reissuance repairs missing index metadata without changing frozen bytes. `--skip-theme-source` skips only the re-read of resolved values from the theme repo (for a machine without it); the warning is printed so the gap stays visible.
...
Resolution: the frozen path is derived from the path and `(vN)`; the frozen file must match its version record and digest in `boards.md` before it is parsed, and carry `data-board-version="N"` and issued status; the section is found by anchor or, failing that, by heading text; an interactive section must be cited with a complete, dependency-valid scenario. A higher working version never satisfies a lower citation. The report says whether the working file is byte-identical to the frozen file, which is the only case where the working file may be opened as a convenience for that citation.
...
This inventories each citation in `boards.md` as `legacy — historical version unavailable` and reserves every number up to the highest cited or stamped one. No historical file is reconstructed. Only citations matching an inventoried path, version, section heading and scenario are exempt from frozen resolution (`cite` reports them as legacy, never as verified). The first new issued number is above the reserved number (`bump` picks it), and every citation written after migration uses the frozen-version contract.
...
node scripts/export-board-bundle.js import <candidate-dir> --into <topic-or-path> --bundle <original-bundle-dir> [--skip-theme-source]
```

A bundle holds `README.md`, `instructions/` (this skill's `SKILL.md` with its `references/`, `templates/` and starter in their relative layout, so the links resolve), `brief.md` (ids assigned), `tokens.md`, `starter.html`, `sources/` (requirement and source excerpts with file path and commit, prepared by the exporter) and `manifest.json`. A **revision bundle** adds `current/<board>.html`, `current/versions/*.html`, `current/boards.md` (that board's entry, with accepted decisions) and manifest fields `baseVersion`, `baseIssued`, `baseDigest`, `baseRevisions` (the complete base revision entries, including their markup) and `frozen` digests. The host starts from the current board, keeps ids and every revision entry, and returns `<board>.html` plus the manifest byte for byte; a candidate cannot claim `data-board-issued="true"`. Existing revision entries must retain their complete markup and relative order, including decision text, author, waiver roles and frozen-file links; record changed decisions in new amendment entries.

`import` requires `--bundle` to name the retained original export, separate from the returned candidate. It first compares the returned manifest byte for byte with that original, then checks the current canonical board before touching anything: a changed working file, a changed or missing frozen file, or a version issued after export is a **stale base** and is rejected without replacing the working file or any issued version (re-export from the current base). A new-board import needs an unused board id and path. Validation uses only the retained bundle's brief and token map after checking their manifest digests; returned candidate copies are ignored. Existing canonical brief/token files must match the export, or the import is rejected as stale. Missing retained inputs or digest mismatches also reject the import without changes. The candidate is then validated with the same structure and token checks as `check`; rejections leave the workspace unchanged. On success the working file is replaced, the brief and token map are placed if missing, and `boards.md` records the import. Issuance still happens on the canonical file after the manual checks.
